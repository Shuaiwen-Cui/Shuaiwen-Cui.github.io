...
2.2 **为什么对 T 做 SVD 就能得到可观性？** 在状态空间模型 (1) 下，用协方差排成的矩阵 \(\mathbf{T}_{1|i}\) 其实可以拆成两个矩阵的乘积：
\(\mathbf{T}_{1|i} = \mathcal{O}_i \, \mathcal{C}_i\)，其中 \(\mathcal{O}_i\) 就是可观性矩阵 (2)，\(\mathcal{C}_i\) 为同样块结构的可控性矩阵。换句话说：T 的「主要方向」就是由 \(\mathcal{O}_i\) 张成的，所以对 \(\mathbf{T}_{1|i}\) 做 SVD、取出前几个主方向（左奇异向量），就得到可观性子空间的一组基。
...
该投影刻画了由过去可预测的那部分未来，其列空间与可观性子空间相关。实际实现中一般不显式形成该矩阵，而是对过去/未来堆叠矩阵做 **RQ** 或 **LQ** 分解，或对加权后的组合做 **SVD**，以因子形式表示投影。
